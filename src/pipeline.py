...
def parse_args():
    parser = argparse.ArgumentParser(description='Preprocesses user-provided metabolomic profiles and Human-GEM, computes novel features integerating these two processed sources, performs downstream classification with the features, and generates benchmark plots.')
...
def main(args):
    ...
    command = 'python3 -W ignore ' + args.script_dir + '/preprocess-metabolome.py' + \
    ...
    command = 'python3 -W ignore ' + args.script_dir + '/preprocess-gem.py' + \
        ' --gem_path ' + args.gem_path + \
    ...
    for react_set_no in [1, 2, 3, 4, 7, 8, 9]:
        ...
        command = 'python3 -W ignore ' + args.script_dir + '/compute-change-feature.py' + \
    ...
    for react_set_no in [2, 4, 7, 8, 9]:
        ...
        command = 'python3 -W ignore ' + args.script_dir + '/compute-ratio-feature.py' + \
            ' --react_set_path ' + react_set_paths[react_set_no] + \
            ' --met_change_path ' + met_change_path + \
            ' --valid_met_path ' + valid_met_path + \
    ...
    for react_set_no in range(1, 10):
        ...
        command = 'python3 -W ignore ' + args.script_dir + '/compute-prob-feature.py' + \
    ...
    #classification
    classification_out_dir = args.out_dir + "/classification"
    classification_log_path = classification_out_dir + "/classification.log"
    command = "python3 -W ignore " + args.script_dir + "/run_classification.py " + \
        " --met_path " + met_out_dir + "/metabolite.tsv" + \
        " --feature_dir " + feature_out_dir + \
        " --case " + args.case + \
        " --control " + args.control + \
        " --script_dir " + args.script_dir + \
        " --out_dir " + classification_out_dir + \
        " --log_path " + classification_log_path    
        
    os.system(command)
    
    #summary
    
    summary_out_dir = args.out_dir + "/summary"
    summary_log_path = summary_out_dir + "/summary.log"
    command = "python3 -W ignore " + args.script_dir + "/summarize_performance.py " + \
        " --in_dir " + classification_out_dir + \
        " --case " + args.case + \
        " --control " + args.control + \
