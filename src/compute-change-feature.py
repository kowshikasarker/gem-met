...
def main(args):
    ...
    change_df[['sample_id', 'sample_group']] = change_df['key'].str.split('.', expand=True)
    
    treatments = set(change_df.sample_group)
    treatments = [treatment for treatment in treatments if not treatment.startswith('No')]
    print('treatments', treatments)
    
    
    
    change_df = change_df.drop(columns=['sample_id', 'sample_group']).set_index('key')
